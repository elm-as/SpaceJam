...
function loop() {
    ...
    if (running) {
        physics.step(timeDirection * 0.016);
        const particles = physics.getParticles();
        const galaxies = physics.galaxies.filter(g => g.alive);
        renderer.updateParticles(particles, galaxies, renderer.camera.position);
    } else if (stepOnce) {
        physics.step(timeDirection * 0.016);
        const particles = physics.getParticles();
        const galaxies = physics.galaxies.filter(g => g.alive);
    ...
    if (now - lastFpsTime >= 500) {
        ...
        ui.updateStats(currentFps, stats.galaxies, stats.stars, stats.bhs, stats.stars * 0.5, physics.getElapsedTime());
    }
}

ui.on('gChange', (val) => { physics.GMultiplier = val; });
ui.on('timeScaleChange', (val) => { physics.tscale = val; });
...
ui.on('toggleTides', (val) => { physics.enableTides = val; });
ui.on('step', () => { stepOnce = true; });
ui.on('multiScene', () => {
    physics.reset();
    startTime = performance.now();
    physics.addMultiGalaxyScene(10);
});
...
// Fusion via UI bottom menu removed (no more Fuse Noyaux button)

ui.on('addRandom', () => {
    const center = getSceneCenter();
    physics.addRandom(center.x, center.y, center.z, 250);
});
...
renderer.renderer.domElement.addEventListener('mouseup', (e) => {
    if (e.button === 0 && ui.placementMode && placementStart) {
        ...
        if (pos) {
            ...
            const vy = dy * -0.005;
            const vz = 0;
            physics.addGalaxy(pos.x, pos.y, pos.z, vx, vy, vz, ui.placementMode, SIZE_PRESETS[1]);
...
document.addEventListener('keydown', (e) => {
    ...
    switch(key) {
        case 'z':
        case 'arrowup':
            renderer.controls.dollyIn(ZOOM_SPEED);
            break;
        case 's':
        case 'arrowdown':
            renderer.controls.dollyOut(ZOOM_SPEED);
            break;
        case 'q':
        case 'arrowleft':
            renderer.controls.rotateLeft(ROTATE_SPEED);
            break;
        case 'd':
        case 'arrowright':
            renderer.controls.rotateRight(ROTATE_SPEED);
            break;
        case 'e':
            renderer.camera.position.y += PAN_SPEED;
            renderer.controls.target.y += PAN_SPEED;
            break;
        case 'a':
            renderer.camera.position.y -= PAN_SPEED;
            renderer.controls.target.y -= PAN_SPEED;
            break;
        case 'f':
            focusOnLargestGalaxy();
            break;
        case 'n':
            // Debug fuse closest nuclei via keyboard (N)
            if (typeof physics.fuseClosest === 'function') {
                physics.fuseClosest(2.0, 0.5, 0.5);
            }
            break;
        case 'tab':
            e.preventDefault();
            ...
            if (galaxies.length > 0) {
                focusedGalaxyIndex = (focusedGalaxyIndex + 1) % galaxies.length;
...
function getSceneCenter() {
    const alive = physics.galaxies.filter(g => g.alive);
...
// Seed initial galaxies (keeps main.js focused on orchestration)
try {
    if (typeof seedInitialGalaxies === 'function') {
        seedInitialGalaxies(physics);
    } else {
        console.warn('[SpaceJam] seedInitialGalaxies non disponible, utilisation seed inline comme fallback');
        physics.addGalaxy(-100, 0, 0, 0, 0, 1.0, 'spiral', SIZE_PRESETS[2]);
        physics.addGalaxy(100, 0, 0, 0, 0, -1.0, 'barred', SIZE_PRESETS[1]);
    }
} catch (e) {
