...
class PhysicsEngine {
    constructor() {
        ...
        this.enableBarnesHut = true;
        this.barnesHutTheta = 0.8;
        this.enableNFWHalo = true;
        this.bhTree = new BarnesHutTree(100000);
    ...
    step(dt) {
        ...
        for (let pass = 0; pass < 2; pass++) {
            ...
            if (this.enableBarnesHut && pass === 0 && this._stepCount % 2 === 0) {
                this._updateBarnesHutForces(alive, Gval);
            }
    ...
    _updateBarnesHutForces(alive, Gval) {
        if (!this.enableBarnesHut) return;
        if (!this._starRefGalaxy) {
            this._starRefGalaxy = [];
            this._starRefStar = [];
        }
        ...
        let totalStars = 0;
        for (let i = 0; i < alive.length; i++) {
            const stars = alive[i].stars;
            for (let j = 0; j < stars.length; j++) {
                if (stars[j].alive) totalStars++;
            }
        }
        if (totalStars < 2) return;

        if (!this._pxBuffer || this._pxBuffer.length < totalStars) {
            const cap = Math.max(totalStars * 2, 40000);
            this._pxBuffer = new Float32Array(cap);
            this._pyBuffer = new Float32Array(cap);
            this._pzBuffer = new Float32Array(cap);
            this._pmassBuffer = new Float32Array(cap);
            this._axBuffer = new Float32Array(cap);
            this._ayBuffer = new Float32Array(cap);
            this._azBuffer = new Float32Array(cap);
        }

        const px = this._pxBuffer;
        const py = this._pyBuffer;
        const pz = this._pzBuffer;
        const pmass = this._pmassBuffer;
        const ax = this._axBuffer;
        const ay = this._ayBuffer;
        const az = this._azBuffer;

        ax.fill(0, 0, totalStars);
        ay.fill(0, 0, totalStars);
        az.fill(0, 0, totalStars);

        const refG = this._starRefGalaxy;
        const refS = this._starRefStar;
        ...
        for (let i = 0; i < alive.length; i++) {
            ...
            for (let j = 0; j < stars.length; j++) {
                ...
                px[idx] = lx * gux + ly * gwx + lz * gox + gx;
                py[idx] = lx * guy + ly * gwy + lz * goy + gy;
                pz[idx] = lx * guz + ly * gwz + lz * goz + gz;
                pmass[idx] = s.size * 0.05;
        ...
        this.bhTree.build(px, py, pz, pmass, idx);
        this.bhTree.computeForces(px, py, pz, idx, ax, ay, az, Gval * 0.15, 1.5, this.barnesHutTheta);

        for (let i = 0; i < idx; i++) {
            const g = refG[i];
            const s = refS[i];
            const fax = ax[i], fay = ay[i], faz = az[i];

            s.lax += fax * g.ux + fay * g.uy + faz * g.uz;
            s.lay += fax * g._wx + fay * g._wy + faz * g._wz;
            s.laz += fax * g.ox + fay * g.oy + faz * g.oz;
        }
    }
