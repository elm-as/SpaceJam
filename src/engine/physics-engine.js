...
class PhysicsEngine {
    ...
    step(dt) {
        ...
        for (let pass=0;pass<2;pass++) {
            ...
            // Recalcul accélérations centres
    ...
    // ── Détection fusion (critère énergie orbitale) ───────────────────────────
    _mergeCheck(Gval) {
        const alive=this.galaxies.filter(g=>g.alive);
        for (let i=0;i<alive.length;i++) {
            for (let j=i+1;j<alive.length;j++) {
                ...
                // Énergie orbitale : E = ½v²_rel − G*M/r
                const Eorb=0.5*relSpeed2 - Gval*mu/(dist+1e-6);

                // Critère 1 : assez proche
                const close = dist < MERGE_DIST;
                // Critère 2 : système lié (E < 0) ET proche
    ...
    // ── Phase de merge : drag de circularisation ──────────────────────────────
    /**
     * Simule la friction dynamique (Chandrasekhar) qui fait perdre de l'énergie orbitale
     * aux noyaux en orbite et les amène inévitablement à fusionner.
     * dragFactor = e^(-λ*dt) avec λ assez grand pour converger en ~1.5s de sim.
     */
    _progressMerges(dt) {
        ...
        for (const m of this._merges) {
            ...
            // Vitesse du CM (conservée pendant le merge)
            ...
            // Interpolation position → CM (de plus en plus vite vers la fin)
    ...
    _merge(a,b) {
        ...
        let tt; if(muU>0.7)tt='elliptical'; else if(muU>0.3)tt=alU<0.4?'elliptical':(Math.random()>0.4?'lenticular':'elliptical'); else tt=a.Mtot>b.Mtot?a.type:b.type;
        ...
        const ms=[];
        const mig=(p)=>{ for (const s of p.stars){
            if (!s.alive) continue;
            ...
            ms.push(new Star(nlx,nly,nlz,nlvx,nlvy,nlvz));
        }};
        mig(a); mig(b);
        merged.stars=ms;
    ...
    _addStarsToGalaxy(galaxy,dt) {
        ...
        for (let i=0;i<count;i++){
            ...
            galaxy.stars.push(new Star(R*Math.cos(theta),R*Math.sin(theta),(Math.random()-0.5)*galaxy.Zh*0.3,
